...
struct AboutCtrl : VirtCtrl {
    ...
    // "Library", directly above showFreqRead
    VirtLink* library = nullptr;
...
// the library start page keeps its own hit targets in win->staticLinks rather
// than a VirtCtrl tree, so it still needs the point -> target lookup
...
static void ShowLibraryHome(VirtMouseEvent* ev) {
    auto* win = (MainWindow*)ev->target->userData;
...
static VirtLink* MkHomeLibraryLink(MainWindow* win) {
    auto* link = new VirtLink(StrL("Library"));
    link->withUnderline = true;
    link->isRtl = IsUIRtl();
    link->userData = (uintptr_t)win;
    link->onClick = MkFunc1Void(ShowLibraryHome);
    return link;
}
...
void DrawAboutPage(MainWindow* win, Gfx* gfx) {
    ...
    if (about->showFreqRead) {
        ...
        link->SetBounds(r);
        if (showLink) {
            libraryShift = txtSize.dy;
        }
    }
    if (!about->library) {
        about->library = MkHomeLibraryLink(win);
        about->AddChild(about->library);
    }
    {
        VirtLink* link = about->library;
        link->font = GetUserGuiFont("MS Shell Dlg", DpiScale(16));
        link->sz = {0, 0};
        Size txtSize = link->GetIdealSize(true);
        Rect r = {0, 0, txtSize.dx, txtSize.dy};
        PositionRB(clientRc, r);
        MoveXY(r, -DpiScale(kInnerPadding), -DpiScale(kInnerPadding) - libraryShift);
        link->SetBounds(r);
    }
...
static void SaveHomeLayoutCache(const HomePageLayout& l, Str filterText, int scrollY) {
    ...
    {
        HomeChromeCtrl* chrome = EnsureHomeChrome(l.win);
        c.rcLibrary = chrome->library ? chrome->library->lastBounds : Rect{};
    }
...
static void ApplyHomeLayoutCache(HomePageLayout& l, int scrollY) {
    ...
    VirtLink* library = chrome->library;
    library->font = fontText;
    library->isRtl = isRtl;
    library->SetBounds(c.rcLibrary);
}
...
static void LayoutHomePage(HomePageLayout& l) {
    ...
    library->font = fontText;
    library->isRtl = isRtl;
    library->sz = {0, 0}; // re-measure: the font may have changed with the DPI
    Size librarySize = library->GetIdealSize(true);
    int libraryGapX = DpiScale(12);

    int openGroupDx = rcIconOpen.dx + 3 + txtSize.dx + libraryGapX + librarySize.dx;
    ...
    Rect rcLibrary(rcOpenDoc.x + rcOpenDoc.dx + libraryGapX, centerY - (librarySize.dy / 2), librarySize.dx,
                   librarySize.dy);
...
static HomeChromeCtrl* EnsureHomeChrome(MainWindow* win) {
    ...
    chrome->library = MkHomeLibraryLink(win);
    chrome->AddChild(chrome->library);

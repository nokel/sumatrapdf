...
// The library index, persisted in SumatraLibrary.txt next to the settings.
// Kept out of GlobalPrefs deliberately: it is derived data that a rescan
// rewrites wholesale, and it is far too big to sit in the settings file.
const libraryPulled: Field[] = [
  field("Key", Str, "", "key of the series or collection the book was pulled out of"),
  field("Name", Str, "", "its display name"),
];

const libraryBook: Field[] = [
  field("Id", Str, "", "stable id for the book; the thumbnail store is keyed by it"),
  field("Title", Str, "", "title as shown in the library"),
  field("Author", Str, "", "author as shown in the library"),
  field("Series", Str, "", "display name of the series the book belongs to"),
  field("SeriesParent", Str, "", ""),
  field("SeriesKey", Str, "", "key of that series, matching one in Series"),
  field("Keys", Str, "", "keys of every collection the book is in, separated by ;"),
  field("Genre", Str, "", ""),
  field("Subgenre", Str, "", ""),
  field("Tags", Str, "", ""),
  field("Path", Str, "", "full path of the document"),
  field("Ext", Str, "", "file extension, without the dot"),
  field("Wiki", Str, "", "key of the lore wiki entry for this book, empty if none"),
  field(
    "TitleSource",
    Str,
    "",
    "where the title came from: filename / pdf-meta / cover / nlp / wikipedia / imdb / openlibrary / googlebooks / wikidata / user / auto; " +
      "user means the user explicitly overrode it and generators must not replace it",
  ),
  field("AuthorSource", Str, "", "where the author came from, same vocabulary as TitleSource"),
  field("YearSource", Str, "", "where the year came from, same vocabulary as TitleSource"),
  field("SeriesSource", Str, "", "where the series came from, same vocabulary as TitleSource"),
  field("Pages", Int, 0, "number of pages, 0 if not worked out yet"),
  field("Year", Int, 0, "year of publication, 0 if unknown"),
  field("Volume", Int, 0, "position in the series, 0 if not part of one"),
  field("BookNlp", Bool, false, "true once BookNLP has analysed the book"),
  field("Cover", Bool, false, "true if the thumbnail store has a cover for this id"),
  array("LibraryOutOf", libraryPulled, "series and collections this book was pulled out of"),
];

const librarySeries: Field[] = [
  field("Key", Str, "", "stable key, referenced by a book's SeriesKey"),
  ...
  field("Head", Str, "", "heading the series is grouped under"),
  field("Subhead", Str, "", "sub-heading within that group"),
  field("Kind", Str, "", "what the grouping is: a series, an author, a collection"),
  field("Guessed", Str, "", "which parts were guessed rather than read from the book"),
  field("Books", Int, 0, "number of books in the series"),
  field("BookNlp", Int, 0, "how many of them BookNLP has analysed"),
  field("Facts", Int, 0, "number of lore facts known about the series"),
  field("Depth", Int, 0, "nesting depth below the top level"),
];

const libraryRoamed: Field[] = [
  field("Mark", Str, "", ""),
  field("LastReadAt", Int64, 0, ""),
  field("TimeSpentMs", Int64, 0, ""),
  field("OpenCount", Int64, 0, ""),
];
...
const libraryStore: Field[] = [
  ...
  array("LibraryRoamed", libraryRoamed, ""),
];

const libraryStoreStruct = struct("LibraryStore", libraryStore, "The library index, persisted in SumatraLibrary.txt");

const coverExample: Field[] = [compactArray("Features", Float, null, ""), field("Good", Bool, false, "")];

const coverChoice: Field[] = [
  field("BookId", Str, "", ""),
  field("Kind", Str, "", ""),
  field("Page", Int, 0, ""),
  compactArray("Rect", Float, null, ""),
  field("FromPath", Str, "", ""),
];

const coverStore: Field[] = [
  field("Version", Int, 1, ""),
  compactArray("Weights", Float, null, ""),
  array("CoverExamples", coverExample, ""),
  array("CoverChoices", coverChoice, ""),
];

const coverStoreStruct = struct("CoverStore", coverStore, "");
...
const libraryStoreHeader = `// !!!!! This file is auto-generated by cmd/gen-settings.ts
...
const coverStoreHeader = `// !!!!! This file is auto-generated by cmd/gen-settings.ts
...
function genLibraryStoreStruct(): string {
  const builtDef: Record<string, number> = {};
  const builtMeta: Record<string, number> = {};
  const structDef = buildStruct(libraryStoreStruct, builtDef);
  const structMetaData = buildMetaData(libraryStoreStruct, builtMeta);

  let content = libraryStoreHeader;
  content = content.replaceAll("{{structDef}}", structDef);
  content = content.replaceAll("{{structMetadata}}", structMetaData);
  return content;
}

function genCoverStoreStruct(): string {
  const builtDef: Record<string, number> = {};
  const builtMeta: Record<string, number> = {};
  const structDef = buildStruct(coverStoreStruct, builtDef);
  const structMetaData = buildMetaData(coverStoreStruct, builtMeta);

  let content = coverStoreHeader;
  content = content.replaceAll("{{structDef}}", structDef);
  content = content.replaceAll("{{structMetadata}}", structMetaData);
  return content;
}
...
export async function main(opts?: { formatOutput?: boolean }) {
  ...
  // Generate the library index header
  {
    let ls = genLibraryStoreStruct();
    ls = ls.replaceAll("\t", "    ");
    const libPath = join("src", "LibraryData.h");
    writeFileMust(libPath, ls);
    if (opts?.formatOutput !== false) {
      const rootDir = join(import.meta.dir, "..");
      await clangFormatFiles(rootDir, [libPath]);
    }
    console.log(`Wrote '${libPath}'`);
  }

  {
    let cs = genCoverStoreStruct();
    cs = cs.replaceAll("	", "    ");
    const coverPath = join("src", "CoverData.h");
    writeFileMust(coverPath, cs);
    if (opts?.formatOutput !== false) {
      const rootDir = join(import.meta.dir, "..");
      await clangFormatFiles(rootDir, [coverPath]);
    }

...
const OUT = process.env.CHUNK19RA_OUT ?? join(ROOT, "scratchpad", "chunk19ra");
const PORT = 7863;
...
function storeCounts(): { total: number; documents: number; entries: number; version: number } {
  try {
    ...
    return { total: -1, documents: -1, entries: -1, version: -1 };
  }
}
...
const DROP_COVER = process.env.CHUNK19RA_DROP_COVER === "1";
...
const REQUIRE_SERVICE = process.env.CHUNK19RA_REQUIRE_SERVICE !== "0";
...
function kindPosts(): string[] {
  return appLogLines().filter((l) => l.includes("ServicePost:") && l.includes("path=/kind"));
}
  ...
  async (client, proc) => {
    ...
    outer: for (const y of ys) {
      for (const x of xs) {
        ...
        const isCard = texts.some((t) => /remove from library/i.test(t)) && texts.some((t) => /play as audio/i.test(t));
    ...
    while (attempt < 6 && !clickedWithService) {
      ...
      const t2 = items.find((i) => /remove from library/i.test(i.text));
      if (!t2) {
        say(`attempt ${attempt}: no Remove from library in the popup`);
        realKeyPress(VK_ESCAPE);
        await sleep(300);
        continue;
      }
    ...
    realMouseClick("left");
    say("clicked Remove from library");
